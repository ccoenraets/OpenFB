...
/**
 * OpenGoogle: Try build on top of OpenOAuth
 */
var createGoogle = function () {

    var GOOGLE_LOGIN_URL = 'https://accounts.google.com/o/oauth2/auth',
    ...
    /**
     * Initialize the OpenGoogle module. You must use this function and initialize the module with an appId before you can
     * use any other function.
     * @param params - init paramters
     *  appId: The id of the Facebook app,
     *  tokenStore: The store used to save the Facebook token. Optional. If not provided, we use sessionStorage.
	...
	var runningInCordova = !!window.cordova || !!window.phonegap;
        ...
        if (runningInCordova) {
	    // Login works with pretty much anything such as http://localhost
            oauthRedirectURL = "http://localhost/index.html";
	    // This is the only url I found works for logout when you do not have your own server
            logoutRedirectURL = "http://localhost/index.html";
        }
	else {
            oauthRedirectURL = baseURL + '/oauthcallback.html';
            logoutRedirectURL = baseURL + '/logoutcallback.html';
	...
	if (!params.loginScope) {
            params['loginScope'] = "profile email";
	}
        loginUrl = GOOGLE_LOGIN_URL + '?client_id=' + googleAppId + '&redirect_uri=' + oauthRedirectURL +
            '&response_type=token&scope=' + params.loginScope;
	console.log(loginUrl);

	if (!params.tokenKey) {
            params['tokenKey'] = 'googletoken';
        }
        openOAuth = createOpenOAuth(params);
    ...
    function login(callback, options) {
        ...
        // Re-calculate loginUrl if specified scope
        if (options && options.loginScope) {
            loginUrlTemp = GOOGLE_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + oauthRedirectURL +
                '&response_type=token&scope=' + params.loginScope;
        } else {
            loginUrlTemp = loginUrl;
        }
     ...
     */
    function logout(callback) {
        var logoutUrl = GOOGLE_LOGOUT_URL + '?access_token=' + openOAuth.getToken() + '&next=' + logoutRedirectURL;
